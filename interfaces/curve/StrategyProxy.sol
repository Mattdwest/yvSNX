...
contract StrategyProxy {
    ...
    // should be noted that the "reward" token is not always the same.
    function claimRewards(address _gauge, address _reward) external {
        require(strategies[msg.sender], "!strategy");
        uint256 _before = IERC20(_reward).balanceOf(address(proxy));
        proxy.execute(mintr, 0, abi.encodeWithSignature("claim_rewards(address)", _gauge));
        uint256 _after = IERC20(_reward).balanceOf(address(proxy));
        uint256 _balance = _after.sub(_before);
        proxy.execute(_reward, 0, abi.encodeWithSignature("transfer(address,uint256)", msg.sender, _balance));
    }
}